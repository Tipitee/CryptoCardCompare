...
export default function ReviewPage() {
  ...
  // dl = content language: be→fr, at→de. Use dl for UI text/content, keep lang for URL prefixes.
  const dl = displayLang(lang);
  const l = L[dl] ?? L.en;
  ...
  const i18n = review ? getReviewI18n(review.slug, dl) : undefined;
  ...
  const reviewMetaTitle =
    ...
    (lang !== 'fr' && review?.cardName
      ? (REVIEW_TITLE_FALLBACK[dl] ?? REVIEW_TITLE_FALLBACK.en)(cardName)
      : review?.metaTitle) ||
    `${l.reviewWord} | TopCryptoCards`;
  const reviewMetaDesc =
    i18n?.metaDescription ||
    (lang !== 'fr' && review?.cardName
      ? (REVIEW_DESC_FALLBACK[dl] ?? REVIEW_DESC_FALLBACK.en)(cardName)
  ...
  const dateLocale = DATE_LOCALE[dl] ?? 'en-GB';
        ...
        <div className="container-app py-10">
          ...
          <div className="flex flex-col md:flex-row gap-8 items-start">
            ...
            <div className="card-surface p-6 w-full md:w-72 shrink-0">
              ...
              >
                {VIEW_CARD_LABEL[dl] ?? VIEW_CARD_LABEL.en}
              </Link>
              ...
              {ISSUER_TO_BRAND[review.issuer] && (() => {
                ...
                return (
                  ...
                  >
                    {SEE_ALL_TIERS[dl] ?? SEE_ALL_TIERS.en}
              ...
              return (
                <div className="p-5 rounded-xl border border-bg-border bg-bg-card">
                  <p className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-3">
                    {THEMED_PAGES_LABEL[dl] ?? THEMED_PAGES_LABEL.en}
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {themes.map(theme => {
                      const slug = REVIEW_THEMATIC_SLUGS[theme]?.[lang] ?? REVIEW_THEMATIC_SLUGS[theme]?.[dl] ?? REVIEW_THEMATIC_SLUGS[theme]?.en;
                      const label = REVIEW_THEMATIC_LABEL[theme]?.[dl] ?? REVIEW_THEMATIC_LABEL[theme]?.en;
            ...
            <div className="lg:sticky lg:top-24 space-y-6">
              ...
              <div className="card-surface p-5 border-cyan-accent/20">
                ...
                <Link to={getRoute('simulator')} className="btn-ghost w-full text-sm flex justify-center border border-bg-border">
                  🧮 {SIMULATOR_LABEL_R[dl] ?? SIMULATOR_LABEL_R.en}
                </Link>
              </div>
